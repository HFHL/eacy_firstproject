...
// API 基础地址（通过 nginx 代理到测试 Agent API）
const API_BASE = '';
...
const OcrViewerPage = () => {
  ...
  const loadHistory = useCallback(async () => {
    ...
    try {
      const response = await fetch(`${API_BASE}/api/test/parse/history`);
      if (response.ok) {
  ...
  const loadHistoryRecord = useCallback(async (record) => {
    ...
    try {
      const response = await fetch(
        `${API_BASE}/api/test/parse/history/${record.id}?parser_type=${record.parser_type}`
      );
      ...
      if (data.original_file) {
        // 使用后端预览接口
        const previewUrl = `${API_BASE}/api/test/file/preview?path=${encodeURIComponent(data.original_file)}`;
  ...
  const handleParse = useCallback(async () => {
    ...
    try {
      ...
      let uploadResponse;
      try {
        uploadResponse = await fetch(`${API_BASE}/api/test/upload`, {
          method: 'POST',
          body: formData
        });
      } catch (networkError) {
        throw new Error(`无法连接后端服务 (${API_BASE})，请确保后端 API 已启动。\n运行命令: cd backend/test/agent/api && python main.py`);
      }
      ...
      let parseResponse;
      try {
        parseResponse = await fetch(`${API_BASE}/api/test/parse`, {
          method: 'POST',
          headers: {
        ...
        throw new Error(`解析请求网络错误: ${networkError.message}`);
      }

      if (!parseResponse.ok) {
        const errorText = await parseResponse.text();
        throw new Error(`解析请求失败 (${parseResponse.status}): ${errorText}`);
      }
        ...
        <div className="control-bar">
          <div className="control-left">
            <Space>
              ...
              <Title level={4} style={{ margin: 0, color: appThemeToken.colorError }}>
        ...
        {historyList.length === 0 ? (
          ...
          <List
            ...
            renderItem={(record) => (
              ...
              >
                <List.Item.Meta
                  avatar={<FileTextOutlined style={{ fontSize: 16, color: record.parser_type === 'textin' ? appThemeToken.colorPrimary : appThemeToken.colorSuccess }} />}
                  title={
                    <Space>
      ...
      <div className="viewer-wrapper">
        {(parsing || documentLoading) ? (
          ...
          <div className="empty-state">
            <ScanOutlined style={{ fontSize: 16, color: appThemeToken.colorText }} />
            <Text type="secondary" style={{ marginTop: 16 }}>
