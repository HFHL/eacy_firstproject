...
const apiService = {
  async getWorkflowLogs() {
    const res = await fetch(`${API_BASE_URL}/api/test/logs`);
    if (!res.ok) throw new Error('Failed to fetch logs');
    return res.json();
  },
  
  async getWorkflowLogDetail(filename) {
    const res = await fetch(`${API_BASE_URL}/api/test/logs/${filename}`);
    if (!res.ok) throw new Error('Failed to fetch log detail');
    return res.json();
  },
  
  getFilePreviewUrl(filePath) {
    return `${API_BASE_URL}/api/test/file/preview?path=${encodeURIComponent(filePath)}`;
  },
  
  async getNotes(taskId) {
    const res = await fetch(`${API_BASE_URL}/api/test/notes/${taskId}`);
    if (!res.ok) throw new Error('Failed to fetch notes');
    return res.json();
  },
  
  async saveNote(taskId, stepId, content) {
    const res = await fetch(`${API_BASE_URL}/api/test/notes`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ task_id: taskId, step_id: stepId, content })
    });
