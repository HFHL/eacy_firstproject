...
const UserProfile = () => {
  ...
  //   if (creditInfo.current < 1000) return { color: appThemeToken.colorError, text: '余额不足' }
  //   if (creditInfo.current < 3000) return { color: appThemeToken.colorWarning, text: '余额偏低' }
  //   return { color: appThemeToken.colorSuccess, text: '余额充足' }
  // }
            ...
            <Row gutter={24}>
              <Col span={8}>
                ...
                <div style={{ marginTop: 8 }}>
                  <Tag color={creditStatus.color === appThemeToken.colorSuccess ? 'green' : creditStatus.color === appThemeToken.colorWarning ? 'orange' : 'red'}>
                    {creditStatus.text}
                  </Tag>
                </div>
              </Col>
              <Col span={8}>
                <Statistic
                  ...
                  suffix="分"
                  valueStyle={{ color: appThemeToken.colorPrimary }}
                />
                <div style={{ marginTop: 8 }}>
              ...
              <Col span={8}>
                <Statistic
                  ...
                  suffix="分"
                  valueStyle={{ color: appThemeToken.colorSuccess }}
                />
                <div style={{ marginTop: 8 }}>
                  <Text type="secondary" style={{ fontSize: 12 }}>
                    ...
                    <List.Item.Meta
                      avatar={
                        <div style={{
                          ...
                          height: 32,
                          borderRadius: '50%',
                          background: item.type === 'earn' ? 'rgba(82, 196, 26, 0.1)' : 'rgba(250, 173, 20, 0.1)',
                          display: 'flex',
                          alignItems: 'center',
                          justifyContent: 'center'
                        }}>
                          {item.type === 'earn' ? (
                            <GiftOutlined style={{ color: appThemeToken.colorSuccess }} />
                          ) : (
                            <FileTextOutlined style={{ color: appThemeToken.colorWarning }} />
                          )}
                        </div>
                      }
                      title={
                        <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                          ...
                          <Text strong style={{ color: item.type === 'earn' ? appThemeToken.colorSuccess : appThemeToken.colorError }}>
            ...
            <Row gutter={24}>
              <Col span={8}>
                <div style={{ textAlign: 'center', padding: 16 }}>
                  <div style={{ fontSize: 24, fontWeight: 'bold', color: appThemeToken.colorPrimary }}>
                    {usageStats.patientsManaged}
                  </div>
                  <div style={{ color: appThemeToken.colorTextTertiary, marginTop: 4 }}>管理患者</div>
                </div>
              </Col>
              <Col span={8}>
                <div style={{ textAlign: 'center', padding: 16 }}>
                  <div style={{ fontSize: 24, fontWeight: 'bold', color: appThemeToken.colorSuccess }}>
                    {usageStats.projectsCreated}
                  </div>
                  <div style={{ color: appThemeToken.colorTextTertiary, marginTop: 4 }}>创建项目</div>
                </div>
              </Col>
              <Col span={8}>
                <div style={{ textAlign: 'center', padding: 16 }}>
                  <div style={{ fontSize: 24, fontWeight: 'bold', color: appThemeToken.colorWarning }}>
                    {usageStats.documentsUploaded}
                  </div>
                  <div style={{ color: appThemeToken.colorTextTertiary, marginTop: 4 }}>上传文档</div>
                </div>
              </Col>
            </Row>
          ...
          {rechargePackages.map(pkg => (
            <Col span={6} key={pkg.id}>
              <Card
                ...
                style={{
                  textAlign: 'center',
                  border: pkg.popular ? `2px solid ${appThemeToken.colorPrimary}` : `1px solid ${appThemeToken.colorBorder}`,
                  position: 'relative'
                }}
              >
                {pkg.popular && (
                ...
                <div style={{ marginBottom: 12 }}>
                  ...
                  <div style={{ fontSize: 24, fontWeight: 'bold', color: appThemeToken.colorPrimary, margin: '8px 0' }}>
                    ¥{pkg.price}
                  </div>
                </div>
                <div style={{ marginBottom: 12 }}>
                  ...
                  {pkg.bonus > 0 && (
                    <div style={{ color: appThemeToken.colorSuccess, fontSize: 12 }}>
