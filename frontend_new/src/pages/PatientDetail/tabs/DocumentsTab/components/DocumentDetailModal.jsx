    ...
    if (!isLinked) {
      const tagContent = (
        <Space size={4}>
          ...
          <span>未绑定患者</span>
          {onArchivePatient && <span style={{ color: appThemeToken.colorPrimary, marginLeft: 2 }}></span>}
        </Space>
      )
      if (onArchivePatient) {
        return (
                                ...
                                <div className="array-summary-list">
                                  {field.arraySummary.map((item, idx) => (
                                    <div key={idx} className="array-summary-item">
                                      <Text style={{ fontSize: 14 }}>{idx + 1}. {item.title}</Text>
            ...
            <Card size="small" title="基本信息" style={{ marginBottom: 16 }}>
              <Space direction="vertical" style={{ width: '100%' }} size={12}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <CalendarOutlined style={{ color: appThemeToken.colorTextTertiary, width: 16 }} />
                  <Text type="secondary">出生日期：</Text>
                  <Text>{selectedPatientForDetail.birth_date || '--'}</Text>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <PhoneOutlined style={{ color: appThemeToken.colorTextTertiary, width: 16 }} />
                  <Text type="secondary">联系电话：</Text>
                  <Text>{selectedPatientForDetail.phone || '--'}</Text>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <IdcardOutlined style={{ color: appThemeToken.colorTextTertiary, width: 16 }} />
                  <Text type="secondary">身份证号：</Text>
                  <Text>{selectedPatientForDetail.id_card || '--'}</Text>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <EnvironmentOutlined style={{ color: appThemeToken.colorTextTertiary, width: 16 }} />
              ...
              {(selectedPatientForDetail.diagnoses && selectedPatientForDetail.diagnoses.length > 0) ? (
                <Space wrap size={[8, 8]}>
                  {selectedPatientForDetail.diagnoses.map((d, i) => (
                    <Tag key={i} color="red" style={{ border: `1px solid ${appThemeToken.colorError}`, marginBottom: 4 }}>
                      {d}
                    </Tag>
                  ))}
                </Space>
