  ...
  return (
    <div
      ...
      style={{
        ...
        borderRadius: '6px',
        background: field.extractable ? appThemeToken.colorFillTertiary : appThemeToken.colorBgContainer,
    ...
    >
      ...
      <div style={{ 
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
          {field.sensitive && (
            <Tooltip title="敏感字段">
              <WarningOutlined style={{ fontSize: 12, color: appThemeToken.colorWarning }} />
            </Tooltip>
          )}
          {!field.editable && (
            <Tooltip title="只读字段">
              <InfoCircleOutlined style={{ fontSize: 12, color: appThemeToken.colorTextTertiary }} />
            </Tooltip>
          )}
        </div>
      </div>
      ...
      {isEditing ? (
        ...
        <div
          style={{
            padding: '8px 12px',
            borderRadius: 4,
            background: field.extractable ? appThemeToken.colorFillTertiary : `${getEhrConfidenceColor(field.confidence)}15`,
            border: field.extractable ? `1px dashed ${appThemeToken.colorBorder}` : `1px solid ${getEhrConfidenceColor(field.confidence)}40`,
            cursor: 'pointer',
            minHeight: 32,
            display: 'flex',
            alignItems: 'center'
        ...
        >
          <Tooltip
            title={
              <div>
                ...
                {field.sensitive && <div style={{ color: appThemeToken.colorWarning }}>⚠️ 敏感字段</div>}
                {!field.editable && <div style={{ color: appThemeToken.colorTextTertiary }}>🔒 只读字段</div>}
          ...
          >
            <Text style={{ 
              fontSize: 14,
              color: field.value ? appThemeToken.colorText : appThemeToken.colorTextTertiary,
