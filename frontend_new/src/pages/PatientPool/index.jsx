...
const PatientPool = () => {
  ...
  const isEditingPatient = !!editingPatientId
  ...
  const handleAddPatient = async () => {
    if (addPatientStep === 0) {
      const requiredFields = isEditingPatient ? ['name', 'gender', 'age'] : ['name', 'gender', 'age', 'department']
      addPatientForm.validateFields(requiredFields).then(() => {
        setAddPatientStep(1)
      }).catch(() => {
      ...
      try {
        ...
        if (values.department) {
          requestData.department_id = values.department
        ...
        const response = isEditingPatient
          ? await updatePatient(editingPatientId, requestData)
          : await createPatient(requestData)
        
        if (response.success && response.code === 0) {
        message.success(isEditingPatient ? '患者信息已更新' : '患者信息已成功添加')
        ...
        console.error(isEditingPatient ? '更新患者失败:' : '创建患者失败:', error)
      } finally {
          ...
          {projects && projects.length > 0 ? (
            <Space direction="vertical" size="small">
              {projects.slice(0, 2).map(project => (
                <Tooltip
                  key={project.id}
                  title={`${project.project_code}: ${project.project_name}（${project.status_label || getProjectStatusDisplayMeta(project.status).label}）`}
                >
                  <Tag 
                    size="small" 
                    color={project.status_color || getProjectStatusDisplayMeta(project.status).color}
    ...
    <div className="page-container fade-in">
      ...
      >
        {!statisticsCollapsed && (
          <Row gutter={[16, 16]}>
            ...
            <Col xs={24} sm={6}>
              <div style={{
                background: appThemeToken.colorSuccess,
                borderRadius: '8px',
                padding: '20px',
                color: 'white'
              }}>
            ...
            <Col xs={24} sm={6}>
              <div style={{
                background: appThemeToken.colorWarning,
                borderRadius: '8px',
                padding: '20px',
                color: 'white'
              }}>
      ...
      <Modal
        ...
        footer={[
          ...
          <Button key="next" type="primary" onClick={handleAddPatient} loading={addPatientLoading} style={{ backgroundColor: appThemeToken.colorPrimary, borderColor: appThemeToken.colorPrimary }}>
            {addPatientStep === 2 ? (isEditingPatient ? '更新' : '保存') : '下一步'}
          </Button>
        ]}
        width={800}
        destroyOnHidden
            ...
            <Row gutter={16}>
              ...
              <Col span={12}>
                <Form.Item 
                  label="所属科室" 
                  name="department" 
                  rules={isEditingPatient ? [] : [{ required: true, message: '请选择科室' }]}
                >
                  <TreeSelect 
                    placeholder="选择科室" 
                    showSearch
