...
import { message, Tabs, Modal, Button, Tooltip } from 'antd';
 ...
 * @returns {Array}
 */
const buildInitialSelectionPath = (data) => {
 ...
 * @param {boolean} props.borderless - 是否使用无边框模式（用于避免外层容器双重边界）
 */
const FormDesigner = forwardRef(({
  ...
  const schemaParser = useSchemaParser({
    ...
    onParseError: (errors, runtimeOptions = {}) => {
      if (runtimeOptions.suppressError) {
        return;
      }
  ...
  const getSelectedObjectByPath = useCallback((path) => {
    ...
    for (const item of path) {
      if (item.type === 'folder') {
        ...
        const source = Array.isArray(currentChild?.children) ? currentChild.children : field.children;
        if (Array.isArray(source)) {
          currentChild = source.find(f => f.id === item.id) || null;
          if (currentChild) {
  ...
  const handleChildSelect = useCallback((fieldId, childFieldId, childPath = []) => {
    ...
    for (const childId of pathIds) {
      const childNode = Array.isArray(cursorChildren)
        ? cursorChildren.find(c => c.id === childId)
        : null;
      if (!childNode) break;
    ...
    // 更新选择路径，支持多级 child 链
    setSelectionPath(prev => {
      const base = prev.filter(p => p.type !== 'child' && p.type !== 'field');
      return [
        ...base,
  ...
  /**
   * 递归更新指定 table 路径下的 children 列表。
   * @param {Object} targetField
   * @param {string[]} tablePath
   * @param {(children: Array) => Array} updater
   * @returns {Object}
   */
  const updateNestedTableChildren = useCallback((targetField, tablePath, updater) => {
    ...
    const applyAtPath = (fieldNode, pathIds) => {
      ...
      if (pathIds.length === 0) {
        return {
          ...fieldNode,
          children: updater([...currentChildren])
        };
      }
  ...
  const handleAddTableChild = useCallback((fieldId, tablePath = []) => {
    ...
    const updatedField = updateNestedTableChildren(field, tablePath, (children) => [...children, createdChild]);
    ...
    const createdNode = locateChildByPath(
      updatedField.children || [],
      [...(Array.isArray(tablePath) ? tablePath : []), createdChild.id]
    );
    if (!createdNode) {
    ...
    const fieldName = field?.name || '表格字段';
    const pathIds = [...(Array.isArray(tablePath) ? tablePath : []), createdChild.id];
    const childSelection = [];
    let cursorChildren = updatedField.children || [];
    for (const childId of pathIds) {
      const childNode = Array.isArray(cursorChildren) ? cursorChildren.find(c => c.id === childId) : null;
      if (!childNode) break;
      childSelection.push({ type: 'child', id: childNode.id, name: childNode.name || childNode.displayName || '新子字段' });
      cursorChildren = childNode.children || [];
    }
    setSelectionPath([
      { type: 'folder', id: folder.id, name: folder.name },
      { type: 'group', id: group.id, name: group.name },
      { type: 'field', id: fieldId, name: fieldName },
      ...childSelection
    ]);
    message.success('表格列已添加');
  }, [data.folders, designData, selectionPath, updateNestedTableChildren]);
  ...
  const handleDeleteTableChild = useCallback((fieldId, childIndex, childPath = []) => {
    ...
    const sourceChildren = parentPath.length === 0
      ...
      : (() => {
          ...
          for (const id of parentPath) {
            cursor = (cursor.children || []).find(c => c.id === id);
            if (!cursor) return [];
          }
    ...
    const deletedChild = targetChildId
      ? sourceChildren.find(c => c.id === targetChildId)
  ...
  }, [data, designData, selectionPath, updateNestedTableChildren]);
  ...
  }, [data.folders, designData, updateNestedTableChildren]);
  ...
  }, [data, designData, syncSelectionPathName, updateNestedTableChildren]);
  ...
  /**
   * 接收目录树当前展开状态，驱动标题栏图标切换。
   * @param {boolean} nextIsAllExpanded 目录树是否已全部展开
   * @param {number} nextFolderCount 目录树分类总数
   */
                ...
                tabBarExtraContent={leftPanelTab === 'structure' ? (
                  <Tooltip title={isFolderTreeAllExpanded ? '收起全部目录' : '展开全部目录'}>
                    <Button
                      ...
                      onClick={handleToggleFolderTreeExpandState}
                    />
                  </Tooltip>
                ) : null}
              >
                <TabPane tab="目录" key="structure">
                  <FolderTree
                    ...
                    onExpandStateChange={handleFolderTreeExpandStateChange}
                    readonly={readonly}
                  />
                </TabPane>
