...
export const useSchemaParser = (options = {}) => {
   ...
   * 解析JSON Schema
   */
  const parseSchema = useCallback((schema, runtimeOptions = {}) => {
    const { suppressSuccess = false, suppressError = false } = runtimeOptions
    setIsParsing(true);
    setParseErrors([]);
    ...
    try {
      ...
      if (!validation.valid) {
        ...
        setIsParsing(false);
        if (onParseError) {
          onParseError(validation.errors, { suppressError });
        }
      ...
      setIsParsing(false);
      if (onParseSuccess) {
        onParseSuccess(designData, { suppressSuccess });
      }
      ...
      setIsParsing(false);
      if (onParseError) {
        onParseError(errors, { suppressError });
      }
   ...
   * 从文件加载Schema
   */
  const loadSchemaFromFile = useCallback((file, runtimeOptions = {}) => {
    return new Promise((resolve, reject) => {
      const reader = new FileReader();
      reader.onload = (e) => {
        try {
          const schema = JSON.parse(e.target.result);
          const result = parseSchema(schema, runtimeOptions);
          resolve(result);
        } catch (error) {
