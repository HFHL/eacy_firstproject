...
import { DISPLAY_TYPES, DEFAULT_CONFIG } from './constants.js';
...
export class SchemaParser {
  ...
  static _parseGroup(name, schema, folderName, enums) {
    ...
    if (isLeafField) {
      const field = this._parseField(name, schema, false, enums);
      return {
    ...
    const group = {
      ...
      isExtractionUnit: this._parseExtractionUnit(target),
    ...
    for (const [fieldName, fieldSchema] of this._orderedPropertyEntries(groupSchema.properties, groupSchema)) {
      ...
      if (fieldSchema.type === 'object' || (fieldSchema.type === 'array' &&
        ...
        if (nestedFields.length > 0) {
          // 创建一个table类型的容器字段
          const _isMultiRow = this._parseTableRows(fieldSchema) === 'multiRow';
          fields.push({
  ...
  static _parseTableRows(schema) {
    const config = this._parseExtendedConfig(schema);
    if (config?.tableRows) {
  ...
  /**
   * 解析抽取单元标记，优先使用显式字段，兼容历史 x-sources 推断。
