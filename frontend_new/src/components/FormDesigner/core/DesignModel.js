...
export class DesignModel {
  ...
  addField(folderId, groupId, fieldData) {
    ...
    if (newField.displayType === DISPLAY_TYPES.TABLE) {
      const tableRows = newField.config?.tableRows
        || (newField.multiRow ? 'multiRow' : 'singleRow');
      newField.isTable = true;
      newField.dataType = 'array';
      newField.multiRow = tableRows === 'multiRow';
      newField.config = {
        ...(newField.config || {}),
        tableRows
      };
      if (!Array.isArray(newField.children)) {
        newField.children = [];
      }
    }
  ...
  updateField(folderId, groupId, fieldId, updates) {
    ...
    updates = this._normalizeTableFieldUpdates(field, updates);
  ...
  updateChildField(folderId, groupId, fieldId, childFieldId, updates) {
    ...
    updates = this._normalizeTableFieldUpdates(childField, updates);
  ...
  /**
   * 规范化表格字段更新，统一 tableRows/multiRow/isTable/children。
