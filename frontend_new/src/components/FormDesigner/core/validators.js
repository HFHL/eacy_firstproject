...
export class SchemaValidator {
  ...
  _validateGroups(properties, folderName) {
    ...
    for (const [groupName, groupSchema] of Object.entries(properties)) {
      // 跳过叶子字段（normalizeSchemaForDesigner 已将其收拢进合成 group，
      // 但作为安全兜底，此处也容忍直接出现的简单类型字段）
  ...
  _validateFields(properties, folderName, groupName) {
    ...
    for (const [fieldName, fieldSchema] of Object.entries(properties)) {
      ...
      // 若字段通过 $ref / allOf / oneOf / anyOf / enum / const 表达类型，
      // 则 type 可以缺失，由 parser 通过 x-display / allOf[0].$ref 推断。
