...
export default function StructuredDataView({
  ...
  const style = useMemo(
    () => ({
      background: dense ? appThemeToken.colorBgContainer : 'transparent',
      border: dense ? `1px solid ${appThemeToken.colorBorder}` : 'none',
      borderRadius: 6,
      padding: dense ? 12 : 0
    }),
    [dense]
  ...
  return (
    <div style={style}>
      <Descriptions
        ...
        layout="vertical"
        styles={{
          label: { color: appThemeToken.colorTextTertiary, fontSize: 12 },
          content: { color: appThemeToken.colorText }
        }}
      >
        {entries.map(([k, v]) => (
