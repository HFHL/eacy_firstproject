...
const UploadFloatingButton = ({ tasks, stats, isUploading, isPaused, onClick }) => {
  ...
  const phaseConfig = {
    uploading: { icon: <LoadingOutlined spin />, color: appThemeToken.colorPrimary, tooltip: `正在上传 ${activeCount} 个文件...` },
    paused:    { icon: <PauseCircleOutlined />,  color: appThemeToken.colorWarning, tooltip: '上传已暂停' },
    failed:    { icon: <WarningOutlined />,       color: appThemeToken.colorError, tooltip: `${stats.failed} 个文件上传失败` },
    allDone:   { icon: <CheckCircleOutlined />,   color: appThemeToken.colorSuccess, tooltip: '全部上传完成' },
    pending:   { icon: <CloudUploadOutlined />,   color: appThemeToken.colorPrimary, tooltip: `${activeCount} 个文件待上传` },
    idle:      { icon: <CloudUploadOutlined />,   color: appThemeToken.colorTextTertiary, tooltip: '上传任务' },
  }
